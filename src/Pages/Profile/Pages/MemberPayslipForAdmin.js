...
const MemberPayslips = (props) => {
  ...
  useEffect(() => {
    ...
    const getPayslips = async () => {
      setLoading(true);
      try {
        const response = await memberApi.getMemberPayslips(props.memberId, {
    ...
    getPayslips();
  }, [page, pageSize, currentCycle, props.memberId]);
    ...
    <div>
      {!isEmpty(payslipId) ? (
        <PayslipDetail
          payslip={payslip}
          admin={true}
          adminNavigate={`/users/${props.memberId}/payslips`}
        />
      ) : (
        <SelfPayslipsTable
          payslips={payslips}
